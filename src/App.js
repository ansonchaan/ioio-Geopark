...
import { fetchAllData, updateGoStart, updateOpenPanel1, updateOpenPanel2, updateShowPanel2, updateInitial } from './reducers';
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    // window.addEventListener("touchstart", this.idle);
    // window.addEventListener("mousedown", this.idle);
  }

  componentWillUnmount() {
    window.removeEventListener("resize", this.onResize);
    // window.removeEventListener("touchstart", this.idle);
    // window.removeEventListener("mousedown", this.idle);
  }
  ...
  //         this.props.dispatch(updateInitial(true));
    ...
    return (
      <Switch>
        <Route
          ...
          render={props => {
            if (/^(en|tc|sc)$/.test(props.match.params.lang)) {
              return <>
                <PopUp {...props} />
                <PageWrap {...props} />
              </>
            } else {
              return <Redirect to={`/tc` + props.location.pathname} />;
            }
          }}
        />
      </Switch>
